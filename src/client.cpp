    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
           ...
           * mpc_x_vals: MPC predicted trajectory
           * mpc_x_vals: waypoints/reference line
           *
           */
          ...
          msgJson["steering_angle"] = res.steer_value;
          msgJson["throttle"] = res.throttle_value;

          //.. add (x,y) points to list here, points are in reference to the vehicle's coordinate system
          // the points in the simulator are connected by a Green line

          msgJson["mpc_x"] = res.mpc_x_vals;
          msgJson["mpc_y"] = res.mpc_y_vals;

          //.. add (x,y) points to list here, points are in reference to the vehicle's coordinate system
          // the points in the simulator are connected by a Yellow line

          msgJson["next_x"] = res.next_x_vals;
          msgJson["next_y"] = res.next_y_vals;
